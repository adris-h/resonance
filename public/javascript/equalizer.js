    ...
    eqPresets.forEach(eqPreset => {
        eqPreset.addEventListener('change', () => {
            ...
            console.log("Looking for preset:", eqPreset.value);
    ...
    function onClickOutside(element1, element2, callback){
        document.addEventListener('click', e => {
            if (!presetSaveBcg.classList.contains('active')) return; console.log("clicked outside");
    ...
    presetSaveButtons.forEach((button) => {
        button.addEventListener("click", (e) => {
            e.stopPropagation();
            if (!presetSaveBcg.classList.contains('active')) {

                presetSaveBcg.classList.add('active');
                setImportState(true);

                // click outside listener only added when opening
    ...
    onAuthStateChanged(auth, async (user) => {
        if (user) {
            ...
            userPresets.forEach((preset) => {
                ...
                console.log("what is this??: ", presets[presetName]);
            ...
            likedPresets.forEach(preset => {
                const presetData = preset.data();
                const presetId = presetData.presetName;


                likedPresets[presetId] = presetData;
                presets[presetId] = presetData.presetData.presetValues;

                console.log("what is this: ", presets[presetId]);
                eqPresets.forEach(preset => {
                    preset.add(new Option(presetData.presetName));
                })
            })
        }
    });
